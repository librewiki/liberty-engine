...
async function doNormal(textToLink, rest, parsingData) {
  ...
  if (await models.Article.existsWithRedirection(textToLink)) {
...
async function doFile(textToLink, rest) {
  const result = await models.Article.findByFullTitleWithRedirection(textToLink);
  if (!result) {
...
//   const exists = await models.Article.existsWithRedirection(textToLink);

  ...
  constructor(text) {
    ...
    lines.forEach((line, i) => {
      ...
      if (line.startsWith('|') || line.startsWith('!')) {
        Table.splitPushCells(line, lastRow.cells);
      } else {
...
<p>`;
  ...
  static splitPushCells(line, cells) {
    const match = line.match(/\|\||!!/u);
    if (match) {
      const idx = match.index;
      cells.push(new Cell(line.slice(0, idx)));
      this.splitPushCells(line.slice(idx + 1), cells);
    } else {
...
module.exports = async (wikitext) => {
  ...
  let stack = [];
  let matched;
  while ((matched = tableRegex.exec(text))) {
    if (matched[0] === '{|') {
      ...
      text =
        ...
        new Table(text.slice(openIdx, closeIdx + 2)).render() +
        text.slice(closeIdx + 2);
      stack = [];

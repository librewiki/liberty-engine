...
class ArgumentNode extends Node {
  ...
  async setArg(parsingData, option) {
    const results
      = await Promise.all(this.argChildren.map(child => child.render(parsingData, option)));
  ...
  async setDefault(parsingData, option) {
    if (this.defaultChildren !== null) {
      const results
        = await Promise.all(this.defaultChildren.map(child => child.render(parsingData, option)));

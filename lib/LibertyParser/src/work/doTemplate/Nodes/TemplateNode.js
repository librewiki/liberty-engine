...
class TemplateNode extends Node {
  ...
  async setTitleAndMagicWord(parsingData, option) {
    const results
      = await Promise.all(this.titleChildren.map(child => child.render(parsingData, option)));
  ...
  async setParams(parsingData, option) {
    ...
    const params
      = await Promise.all(this.paramChildren.map(child => child.render(parsingData, option)));

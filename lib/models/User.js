...
class User extends LibertyModel {
  ...
  static async initialize() {
    this.anonymous = new User({
      ...
      email: null,
      emailConfirmed: true,
      roles: [await models.Role.findOne({ where: { name: 'anonymous' } })],
    }, {
      include: [models.Role],
    });
  }
  ...
  static async signUp({ email, password, username }) {
    const loggedIn = await models.Role.findOne({ where: { name: 'loggedIn' } });
...
${models.Setting.get('domain')}/mail-confirm?username=${encodeURIComponent(username)}&code=${confirmCode}`,
      })
        .catch((err) => {
          console.error(err);
  ...
  async issueToken() {
    ...
    return jwtSignAsync(payload, secret, { expiresIn: '30min' });
  }

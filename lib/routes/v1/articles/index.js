  ...
  async (req, res, next) => {
    try {
      ...
      if (fields.includes('permissions')) {
        promises.push(article.getArticlePermissions().then((permissions) => {
...
router.put(
  ...
  [
    ...
    body('permissions.*.readable')
      .optional()
      .custom(v => [true, false, null].includes(v)),
    body('permissions.*.editable')
      .optional()
      .custom(v => [true, false, null].includes(v)),
    body('permissions.*.renamable')
      .optional()
      .custom(v => [true, false, null].includes(v)),
    body('permissions.*.deletable')
      .optional()
      .custom(v => [true, false, null].includes(v)),
  ],
  [
  ...
  async (req, res, next) => {
    try {
      ...
      await sequelize.transaction(async (t) => {
        ...
        await ArticlePermission.bulkCreate(permissions, { transaction: t });
        return new Response.Success().send(res);
      });
    } catch (err) {
      return next(err);
